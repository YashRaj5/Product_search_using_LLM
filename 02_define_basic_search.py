...
# MAGIC %pip install sentence-transformers==2.2.2 langchain==0.0.179 chromadb==0.3.25 typing-inspect==0.8.0 typing_extensions==4.5.0 --upgrade mlflow
...
# persist vectordb to the storage
vectordb.persist()

# COMMAND ----------

# MAGIC %md
# MAGIC From a count of the vector database collection, we can see that every product entry from our original dataframe has been loaded:
...
# MAGIC %md
# MAGIC We can also take a peek at one of the records in the database to see how our data has been transformed. Details about our product id and product name, basically all the fields in the original dataframe not identified as the document are stored in the Metadatas field. The text we identified as our document is visible in its original form through the Documents field and the embedding created from this is available through the embeddings field:
...
# MAGIC %md
# MAGIC # Demonstrate Basic Search Capabilities
# MAGIC To get a sense of how our search will work, we can perofrm a similarity search on our vector database:

# COMMAND ----------

# DBTITLE 1,Perform Simple Search
display(vectordb.similarity_search_with_score('kid-proof rug'))
...
# MAGIC %md
# MAGIC # Persist Model for Deployment
...
# MAGIC %md
# MAGIC The load_context of the previously defined wrapper class addresses the steps that need to take place at model initialization. Two of those steps make reference to artifacts within the model's context.
# MAGIC
...
# MAGIC %md
# MAGIC Next, we need to identify the packages we need to insure are installed as our model is deployed. These are:
...
# add the required packages to envrionements configuraiton
...
# MAGIC %md 
# MAGIC Now we can persist our model to MLflow. Notice that in this scenario, our embedding model and Chroma database are being loaded as artifacts and that our python_model is just the class definition that provides the logic for hydrating a model from those artifacts:
...
# MAGIC %md
# MAGIC We can now elevate our model to production status. This would typically be done through a careful process of testing and evaluation but for the purposes of this demo, we'll just programmatically push it forward:
...
# MAGIC %md
# MAGIC Loading our model, we can perform a simple test to see results from a sample search.
...
# MAGIC %md 
# MAGIC If you are curious why we are constructing a pandas dataframe for our search term, please understand that this aligns with how data will eventually passed to our model when we host it in model serving. The logic in our predict function anticipates this as well.
# MAGIC
